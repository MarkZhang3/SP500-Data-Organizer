...
wrds_ciq_key = pd.read_csv(home_dir + wrds_ciq_key_file, nrows=5000)
# print(keys_df['symboltypename'].unique())
print(wrds_ciq_key.columns)
wrds_ciq_key.to_csv(home_dir + '/wrds_ciq_key_test.csv', index=False)
...
# add code name from wrds, subsidiary name 
# make column M and A year, make effective or closed date 
# if neither available, take transaction announced date, convert to year 
# collapse M and A transactions, want to identify 
# rearrange dataset to put company name, parent name first 

# create start year, end year columns, set start year to whenever theres m and a transaction year (closed or effective)
# leave as NA otherwise 
# if theres movement from one parent to another, have a start year, end year for when company was part of first parent and when part of second company
